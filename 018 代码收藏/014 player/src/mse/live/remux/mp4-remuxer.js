...
//              }
    ...
    _generateSilentAudio(dts, frameDuration) {
        Global.debug(this.TAG, `GenerateSilentAudio: dts = ${dts}, duration = ${frameDuration}`);

        let unit = AAC.getSilentFrame(this._audioMeta.channelCount);
        if (unit == null) {
            Global.debug(this.TAG, `Cannot generate silent aac frame for channelCount = ${this._audioMeta.channelCount}`);
            return null;
        }
