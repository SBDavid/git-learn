...
class MSStreamLoader extends BaseLoader {
    ...
    static isSupported() {
        try {
           ...
           Global.debug('MSStreamLoader', e.message);
            return false;
        }
    }
    ...
    _internalOpen(dataSource, range, isSubrange) {
        ...
        reader.onprogress = this._msrOnProgress;
        reader.onload = this._msrOnLoad;
        reader.onerror = this._msrOnError;

        let xhr = this._xhr = new XMLHttpRequest();
        xhr.open('GET', seekConfig.url, true);
        xhr.responseType = 'ms-stream';
        xhr.onreadystatechange = this._xhrOnReadyStateChange;
    ...
    _msrOnProgress = (e) => {
        ...
        if (bigbuffer.byteLength >= this._bufferLimit) {
            Global.debug(this.TAG, `MSStream buffer exceeded max size near ${byteStart + slice.byteLength}, reconnecting...`);
            this._doReconnectIfNeeded();
        }
    }
